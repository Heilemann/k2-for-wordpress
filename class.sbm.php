...
define('SBM_PATH', dirname(__FILE__) );
define('SBM_URL', WP_PLUGIN_URL . '/' . basename(dirname(__FILE__)) );
...
class K2SBM {
	function init() {
		...
		if(is_admin()) {
			...
			if (isset($_POST['sbm_action']) && $_POST['sbm_action'] == 'restore' && $_FILES['backup']['error'] == 0) {
		...
		if ( version_compare( get_option('k2sbm_version'), SBM_VERSION, '<' ) )
			K2SBM::install();
	...
	function install() {
		add_option('k2sbm_version', SBM_VERSION, 'The current sidebar module version.');
	...
	function module_admin() {
		...
		<?php
			...
			include(SBM_PATH . '/display/sbm-admin.php');
		}
	...
	function module_admin_scripts() {
		wp_register_script('jquery-dimensions',
			SBM_URL . '/js/jquery.dimensions.js',
			array('jquery'), '1.2');

		wp_register_script('jquery-easing',
			SBM_URL . '/js/jquery.easing.js',
			array('jquery'), '1.1.2');

		wp_register_script('humanmsg',
			SBM_URL . '/js/jquery.humanmsg.js',
			array('jquery', 'jquery-easing'), '1.0-RC7');

		wp_register_script('humanundo',
			SBM_URL . '/js/jquery.humanundo.js',
			array('jquery'), '1.0-RC7');

		wp_register_script('sbm',
			SBM_URL . '/js/sbm.js',
			array('jquery', 'interface', 'jquery-dimensions', 'humanmsg', 'humanundo'), SBM_VERSION);

		// Add our script to the queue
		wp_enqueue_script('sbm');
	}

	function module_admin_head() {
		?>
			<link type="text/css" rel="stylesheet" href="<?php echo SBM_URL; ?>/css/sbm.css" />
			<link type="text/css" rel="stylesheet" href="<?php echo SBM_URL; ?>/css/humanmsg.css" />
	...
	function module_scan() {
		K2SBM::module_scan_dir(SBM_PATH . '/modules/');
	}
...
class k2sbmSidebar {
	...
	function k2sbmSidebar($id, $name, $before_module, $after_module, $before_title, $after_title) {
		...
		// Load the modules
		if(isset($k2sbm_active_modules[$this->id])) {
...
class k2sbmModule {
	...
	function displayControl() {
		...
		include(SBM_PATH . '/display/edit-module-form.php');
	...
	function displayPostList() {
		...
		extract(array('module' => $this));
		include(SBM_PATH . '/display/edit-module-posts-form.php');
	}

	function displayPageList() {
		// Display the generic post list
		extract(array('module' => $this));
		include(SBM_PATH . '/display/edit-module-pages-form.php');
	}
