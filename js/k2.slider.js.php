...
jQuery.k2Slider = {
		...
		return this.each(
			function()
			{
				this.slideCfg.sliders.each(
					function (key) 
					{ 
						jQuery.k2Slider.dragmoveBy(this,values[key]);
					}
				);
			}
		);
	...
	dragmoveByKey : function (event)
	{
		...
		switch (pressedKey)
		{
			//end
			case 35:
				jQuery.k2Slider.dragmoveBy(this.dragElem, [2000, 2000] );
			break;
			//home
			case 36:
				jQuery.k2Slider.dragmoveBy(this.dragElem, [-2000, -2000] );
			...
			//left
			case 37:
				jQuery.k2Slider.dragmoveBy(this.dragElem, [-this.dragElem.dragCfg.gx||-1, 0] );
			break;
			//up
			case 38:
				jQuery.k2Slider.dragmoveBy(this.dragElem, [0, -this.dragElem.dragCfg.gy||-1] );
			break;
			//right
			case 39:
				jQuery.k2Slider.dragmoveBy(this.dragElem, [this.dragElem.dragCfg.gx||1, 0] );
	...
	dragmoveBy : function (elm, position) 
	{
		...
		jQuery.k2Slider.modifyContainer(elm);		
		...
		if (elm.dragCfg.si && (elm.dragCfg.onSlide || elm.dragCfg.onChange)) {
			jQuery.k2Slider.onSlide(elm, nx, ny);
		}
	...
	build : function(o) {
		return this.each(
			function()
			{
				...
				toDrag.keydown(jQuery.k2Slider.dragmoveByKey);
				toDrag.attr('tabindex',jQuery.k2Slider.tabindex++);	
				...
				if (o.values && o.values.constructor == Array) {
					for (i = o.values.length -1; i>=0;i--) {
						if (o.values[i].constructor == Array && o.values[i].length == 2) {
							...
							if (el.tagName) {
								jQuery.k2Slider.dragmoveBy(el, o.values[i]);
							}
						}
					}
				}
...
jQuery.fn.extend(
	{
		 ...
		 * @author Stefan Petre
		 */
		newPageSlider : jQuery.k2Slider.build,
		/**
		 ...
		 * @author Stefan Petre
		 */
		setPageSlider : jQuery.k2Slider.set,
		/**
		 ...
		 * @author Stefan Petre
		 */
		getPageSlider : jQuery.k2Slider.get
	}
);