...
class K2Styles {
	function init() {
		/* Possibility Deprecated - to keep styles as pure css and child themes for php
	...
	function set_styles_dir( $path = false ) {
		...
		*/
		...
		$dir = str_replace(
					array( '%k2%', '%child%', '%content%', '%root%' ),
					array( TEMPLATEPATH, STYLESHEETPATH, WP_CONTENT_DIR, untrailingslashit(ABSPATH) ),
					$path
				);
	...
	function set_styles_url( $path = false ) {
		...
		$url = str_replace(
					array( '%k2%', '%child%', '%content%', '%root%' ),
					array( get_template_directory_uri(), get_stylesheet_directory_uri(), content_url(), site_url() ),
					$path
				);
...
<?php /*
					<tr class="simple-option">
						<th scope="row">
							<label for="k2-style"><?php _e('Active Style:', 'k2_domain'); ?></label>
						</th>
						<td>
...
<?php
	...
	function theme_editor_append_styles() {
		...
		if ( ('theme-editor.php' == $pagenow) and strpos(K2Styles::get_styles_dir(), WP_CONTENT_DIR) !== false ) {
			get_themes();
			$current = get_current_theme();

			// Get the path relative to wp-content
			$style_path = str_replace(WP_CONTENT_DIR, '', K2Styles::get_styles_dir());

			// Get a list of style css
			$styles = K2::files_scan( K2Styles::get_styles_dir(), 'css', 2 );;
