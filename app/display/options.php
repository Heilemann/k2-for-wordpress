...
<div class="wrap">
	<form name="dofollow" action="" method="post">
		...
		<div class="configstuff">
			...
			<p><input id="k2-advnav" name="k2[advnav]" type="checkbox" value="1" <?php checked('1', get_option('k2livesearch')); ?> /> <?php _e('Enable Advanced Navigation') ?></p>

			<p><small><?php _e('K2\'s Advanced Navigation is in reality a couple of features which, when combined, work to make the task of searching through your blog faster and easier. This includes inline AJAX-powered livesearch as well as the ability to flip back and forth between archive pages, without ever reloading the page.') ?></small></p>



			<h3><?php _e('Archives Page','k2_domain'); ?></h3>

			<p><input id="k2-archives" name="k2[archives]" type="checkbox" value="add_archive" <?php checked('add_archive', get_option('k2archives')); ?> /> <?php _e('Enable Archives Page') ?></p>
			...
			<?php if (!function_exists('af_ela_set_config')) { ?>
				<?php printf(__('We highly recommend that you install %s','k2_domain'), '<a href="http://www.sonsofskadi.net/index.php/extended-live-archive/">' . __('Arnaud Froment\'s Extended Live Archives','k2_domain') . '</a>' . __(' for maximum archival pleasure.') ) ?></small></p>
			<?php } else { ?>
			...
			<p><input id="k2-livecommenting" name="k2[livecommenting]" type="checkbox" value="1" <?php checked('1', get_option('k2livecommenting')); ?> /> <?php _e('Enable Live Commenting') ?></p>
				
			<p><small><?php _e('Live comments use AJAX to submit comments to the server without reloading the page, making the experience more seamless for the user.'); ?></small></p>
			...
			<p><small><?php _e('Your header is the crown of your blog, with plenty of room to express yourself. Through here you can upload and manipulated images for use as header backgrounds, as well as decide whether to it\'s blog or journal.') ?></small></p>

			<table>
			<?php if ($is_header_dir) { ?>

			<tr><td>
				<small><?php _e('Upload an Image') ?></small>
			</td><td>
				<input id="picture" name="picture" type="file" disabled="disabled" />
				<input id="upload-activate" name="upload_activate" type="checkbox" value="1"<?php if (!$is_header_dir) { ?> disabled="disabled"<?php } ?> />
				<label for="upload-activate"><small><?php _e('Activate immediately','k2_domain'); ?></small></label>
			</td></tr>

			<tr><td style="width: 160px">
				<small><?php _e('Select an Image') ?></small>
			</td><td>	
			...
			<tr><td>
				<small><?php _e('Randomize Header Image') ?></small>
			</td><td>
			...
			<tr><td style="width: 160px">
				<small><?php _e('Rename the \'Blog\' tab') ?></small>
			</td><td>
