...
<head profile="http://gmpg.org/xfn/11">
	<title><?php wp_title(''); if (function_exists('is_tag') && is_tag()) { ?>Tag Archive for <?php echo $tag; } if (is_archive()) { ?> archive<?php } elseif (is_search()) { ?> Search for <?php echo $s; } if ( !(is_404()) && (is_search()) || (is_single()) || (is_page()) || (function_exists('is_tag') && is_tag()) || (is_archive()) ) { ?> at <?php } ?> <?php bloginfo('name'); ?></title>
	...
	<?php /* Live Commenting */ if ((get_option('k2livecommenting') == 1) && ((is_page() || is_single()) && (!isset($_GET['jal_edit_comments'])) && ('open' == $post-> comment_status) || ('comment' == $post-> comment_type) )) { ?>
	...
	<?php /* Hide Author Elements */ if (!is_user_logged_in() && (is_page() || is_single()) && ($comment_author = $_COOKIE['comment_author_'.COOKIEHASH]) && ('open' == $post-> comment_status) || ('comment' == $post-> comment_type) ) { ?>
...
<body class="k2<?php if (get_option('k2widthtype') == 0) echo ' flex'; if (is_single()) echo ' permalink'; if (is_page()) echo ' staticpage'; ?>"<?php if (get_option('permalink_structure') != '' && is_page()) { echo ' id="'.get_query_var('name').'"'; } ?>>
...
<div id="page">
	...
	<div id="header">
		...
		<ul class="menu">
			<li class="<?php if ( is_home() || is_archive() || is_single() || is_paged() || is_search() || (function_exists('is_tag') && is_tag()) ) { ?>current_page_item<?php } else { ?>page_item<?php } ?>"><a href="<?php echo get_settings('home'); ?>/"><?php echo get_option('k2blogornoblog'); ?></a></li>
